...
def display_records():
    ...
    if choice==1:
        ...
        if choice2==1:
            ...
            if found == False:
                print("No students found in this CGPA range.")
    else:
...
def sort_student():
    ...
    if sort =="1":
        ...
        order=int(input("Choose the order of sorting:\n1. Ascending Order\n2. Descending Order\n3. Display the top 5 students\n"))
        if order==1:
            print("SORTING BY CGPA in ASCENDING ORDER....\n")
            students.sort(key=lambda student:float((student["cgpa"])))
            for student in students:
                print(f"USN: {student['usn']}\nName: {student['name']}\nAge: {student['age']}\nProgramme: {student['programme']}\nCGPA: {student['cgpa']}\n")
        elif order==2:
            print("SORTING BY CGPA in DESCENDING ORDER....\n")
            students.sort(key=lambda student:float((student["cgpa"])), reverse=True)
            for student in students:
                print(f"USN: {student['usn']}\nName: {student['name']}\nAge: {student['age']}\nProgramme: {student['programme']}\nCGPA: {student['cgpa']}\n")
        elif order==3:
            print("DISPLAYING THE TOP 5 STUDENTS....\n")
            students.sort(key=lambda student:float((student["cgpa"])), reverse=True)
            for i in range(min(5,len(students))):
                student=students[i]
                print(f"USN: {student['usn']}\nName: {student['name']}\nAge: {student['age']}\nProgramme: {student['programme']}\nCGPA: {student['cgpa']}\n")
        else:
            print ("Please enter the number 1, the number 2 or the number 3 and try again.")
    else:
